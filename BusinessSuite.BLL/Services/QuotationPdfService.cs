...
{
    public void GenerateQuotation(Quotation quotation, string filePath)
    {
        QuestPDF.Settings.License = LicenseType.Community;

        bool isGstRegistered = quotation.Business?.IsGSTRegistered == true;
        bool isComposition = quotation.Business?.GstType == BusinessGstType.Composition;
        bool isInterState = isGstRegistered && !isComposition && quotation.TotalIGST > 0;

        void GenerateQuotationLayout(ColumnDescriptor column, string copyType)
        {
            // --- CENTERED HEADING & COPY INDICATOR ---
            string title = "QUOTATION";
            column.Item().Row(row =>
            {
                row.RelativeItem().Text(title).FontSize(10).SemiBold().FontColor(Colors.Grey.Darken2);
                row.RelativeItem().AlignRight().Text(copyType).FontSize(8).Bold().FontColor(Colors.Grey.Darken1);
            });
            ...
            column.Item().Border(0.5f).Row(row =>
            {
                ...
                {
                    c.Item().Text(quotation.Business?.BusinessName?.ToUpper() ?? "").FontSize(12).ExtraBold();
                        
                    c.Spacing(1); // Small gap between name and address

                    c.Item().MaxWidth(200).Text(quotation.Business?.Address ?? "");
                    c.Item().Text($"{quotation.Business?.State}, India.");

                    c.Item().Text(x => { 
                        x.Span("Phone: ").FontSize(9).SemiBold(); 
                        x.Span(quotation.Business?.ContactNo ?? "-").FontSize(9); 
                    });
                    
                    if (quotation.Business?.IsGSTRegistered == true)
                    {
                        c.Item().Text(x => { 
                            x.Span("GSTIN: ").FontSize(9).SemiBold(); 
                            x.Span(quotation.Business?.GSTIN ?? "").FontSize(9); 
                        });
                    }
                    if (!string.IsNullOrWhiteSpace(quotation.Business?.PAN))
                    {
                        c.Item().Text(x => { 
                            x.Span("PAN: ").FontSize(9).SemiBold(); 
                            x.Span(quotation.Business.PAN).FontSize(9); 
                        });
                    }
                });

                // Right Side: Quotation Metadata (Shaded background for contrast)
                row.RelativeItem().Padding(10).Column(c =>
                {
                    c.Item().Row(r =>
                    {
                        r.RelativeItem().Text("Quotation No:").FontSize(9).SemiBold();
                        r.RelativeItem().AlignRight().Text(quotation.QuotationNumber).FontSize(9).Bold();
                    });

                    c.Item().Row(r =>
                    {
                        r.RelativeItem().Text("Date:").FontSize(9).SemiBold();
                        r.RelativeItem().AlignRight().Text(quotation.QuotationDate.ToString("dd MMM yyyy")).FontSize(9);
                    });

                    c.Item().Row(r =>
                    {
                        r.RelativeItem().Text("Valid Until:").FontSize(9).SemiBold();
                        r.RelativeItem().AlignRight().Text(quotation.DueDate?.ToString("dd MMM yyyy") ?? "-").FontSize(9);
                    });
                    ...
                    c.Item().Text(quotation.PaymentTerms ?? "Not Specified").FontSize(9).Italic();
                });
            });
            ...
            column.Item().BorderLeft(0.5f).BorderRight(0.5f).BorderBottom(0.5f).Row(row =>
            {
                row.RelativeItem().Padding(5).Column(c =>
                {
                    ...
                    c.Item().PaddingLeft(4).Text(quotation.Customer?.CustomerName ?? "").SemiBold();
                    c.Item().PaddingLeft(4).MaxWidth(220).Text(quotation.Customer?.BillingAddress ?? "");
                    c.Item().PaddingLeft(4).Text($"{quotation.Customer?.State}, India.");
                    
                    // Only show GSTIN if customer is registered for GST
                    if (quotation.Customer?.GstTreatment != "Unregistered" && !string.IsNullOrWhiteSpace(quotation.Customer?.GSTIN))
                    {
                        c.Item().PaddingLeft(4).Text(x => { x.Span("GSTIN: ").SemiBold(); x.Span(quotation.Customer?.GSTIN ?? ""); });
                    }
                });
                ...
                row.RelativeItem().BorderLeft(0.5f).Padding(5).Column(c =>
                {
                    ...
                    c.Item().PaddingLeft(4).Text(quotation.Customer?.ShippingAddress ?? quotation.Customer?.BillingAddress ?? "");
                    c.Item().PaddingLeft(4).Text(x => { x.Span("Place of Supply: ").SemiBold(); x.Span(quotation.PlaceOfSupply ?? ""); });
                });
            });
            ...
            column.Item().Table(table =>
            {
                ...
                foreach (var (item, index) in quotation.Items.Select((v, i) => (v, i)))
                {
                    ...
                    table.Cell().Element(CellStyle).Text(!string.IsNullOrWhiteSpace(item.Description) ? item.Description : (item.Product?.ProductName ?? ""));
            ...
            column.Item().BorderLeft(0.5f).BorderRight(0.5f).BorderBottom(0.5f).Row(row =>
            {
                row.RelativeItem(6).Padding(5).Column(c =>
                {
                    c.Item().Text(x =>
                    {
                        ...
                        x.Span(NumberToWordsConverter.ConvertToWords(quotation.GrandTotal).ToUpper());
                    });

                    c.Item().PaddingTop(8).Text("BANK DETAILS").SemiBold().FontSize(7).Underline();
                    c.Item().Text(x => { x.Span("Bank: ").SemiBold(); x.Span(quotation.Business?.BankName ?? ""); });
                    c.Item().Text(x => { x.Span("Name: ").SemiBold(); x.Span(quotation.Business?.AccountName ?? ""); });
                    c.Item().Text(x => { x.Span("A/c No: ").SemiBold(); x.Span(quotation.Business?.AccountNumber ?? ""); });
                    c.Item().Text(x => { x.Span("IFSC: ").SemiBold(); x.Span(quotation.Business?.IFSC ?? ""); });
                });
                ...
                row.RelativeItem(4).BorderLeft(0.5f).Column(c =>
                {
                    ...
                    if (isGstRegistered && !isComposition)
                        AddSummaryRow("Sub Total", quotation.TotalAmount.ToString("N2"));

                    if (isGstRegistered && !isComposition)
                    {
                        if (isInterState)
                            AddSummaryRow("Total IGST", quotation.TotalIGST.ToString("N2"));
                        else
                        {
                            AddSummaryRow("Total CGST", quotation.TotalCGST.ToString("N2"));
                            AddSummaryRow("Total SGST", quotation.TotalSGST.ToString("N2"));
                        }
                    }

                    if (quotation.Discount > 0)
                        AddSummaryRow("Discount", $"-{quotation.Discount:N2}");

                    AddSummaryRow("Shipping", quotation.ShippingCharges.ToString("N2"));
                    AddSummaryRow("Round Off", quotation.RoundOff.ToString("N2"));

                    // Grand Total Row
                    c.Item().Background(Colors.Grey.Lighten2).Padding(5).Row(r =>
                    {
                        r.RelativeItem().Text("Grand Total").FontSize(10).ExtraBold();
                        r.RelativeItem().AlignRight().Text($"₹ {quotation.GrandTotal:N2}").FontSize(10).ExtraBold();
                    });
                });
            });
            ...
            column.Item().BorderLeft(0.5f).BorderRight(0.5f).BorderBottom(0.5f).Row(row =>
            {
                row.RelativeItem().Padding(5).Column(c =>
                {
                    ...
                    c.Item().Text(quotation.TermsAndConditions ?? "1. Subject to local jurisdiction.").FontSize(7);
                });
                row.RelativeItem().BorderLeft(0.5f).AlignRight().Padding(5).Column(c =>
                {
                    c.Item().Text($"For {quotation.Business?.BusinessName}").SemiBold();
        ...
        Document.Create(container =>
        {
            ...
            container.Page(page =>
            {
                ...
                {
                    GenerateQuotationLayout(column, "CUSTOMER COPY");
                });
            ...
            container.Page(page =>
            {
                ...
                page.Content().Column(column =>
                {
                    GenerateQuotationLayout(column, "OFFICE COPY");
                });
