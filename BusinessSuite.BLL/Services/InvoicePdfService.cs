...
public class InvoicePdfService
{
    public void GenerateInvoice(Invoice invoice, string filePath)
    {
        ...
        void GenerateInvoiceLayout(ColumnDescriptor column, string copyType)
        {
            ...
            column.Item().Border(0.5f).Row(row =>
            {
                ...
                row.RelativeItem().BorderRight(0.5f).Padding(10).Column(c =>
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(invoice.Business?.PAN))
                    {
                        c.Item().Text(x => { 
                            x.Span("PAN: ").FontSize(9).SemiBold(); 
                            x.Span(invoice.Business.PAN).FontSize(9); 
                        });
                    }
                });
        ...
        Document.Create(container =>
        {
            ...
            container.Page(page =>
            {
                ...
                {
                    GenerateInvoiceLayout(column, "ORIGINAL FOR RECIPIENT");
                });
            ...
            container.Page(page =>
            {
                ...
                page.Content().Column(column =>
                {
                    GenerateInvoiceLayout(column, "DUPLICATE FOR OFFICE");
                });
