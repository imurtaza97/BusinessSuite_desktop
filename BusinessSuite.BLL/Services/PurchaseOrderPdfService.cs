...
public class PurchaseOrderPdfService
{
    public void GeneratePO(PurchaseOrder po, string filePath)
    {
        ...
        void GeneratePOLayout(ColumnDescriptor column, string copyType)
        {
            // --- CENTERED HEADING & COPY INDICATOR ---
            column.Item().Row(row =>
            {
                row.RelativeItem().Text("PURCHASE ORDER").FontSize(10).SemiBold().FontColor(Colors.Grey.Darken2);
            ...
            column.Item().Border(0.5f).Row(row =>
            {
                ...
                row.RelativeItem().BorderRight(0.5f).Padding(10).Column(c =>
                {
                    ...
                    if (po.Business?.IsGSTRegistered == true)
                    {
                        c.Item().Text(x => { 
                    ...
                    if (!string.IsNullOrWhiteSpace(po.Business?.PAN))
                    {
                        c.Item().Text(x => { 
                            x.Span("PAN: ").FontSize(9).SemiBold(); 
                            x.Span(po.Business.PAN).FontSize(9); 
                        });
                    }
                });

                // Right Side: PO Metadata (Shaded background for contrast)
                row.RelativeItem().Padding(10).Column(c =>
                {
                    ...
                    c.Item().Row(r =>
                    {
                        r.RelativeItem().Text("PO Date:").FontSize(9).SemiBold();
                        r.RelativeItem().AlignRight().Text(po.PODate.ToString("dd MMM yyyy")).FontSize(9);
                    });

                    c.Item().Row(r =>
                    {
                        r.RelativeItem().Text("Expected Date:").FontSize(9).SemiBold();
            ...
            column.Item().BorderLeft(0.5f).BorderRight(0.5f).BorderBottom(0.5f).Row(row =>
            {
                row.RelativeItem().Padding(5).Column(c =>
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(po.Vendor?.GSTIN))
                    {
            ...
            column.Item().Table(table =>
            {
                table.ColumnsDefinition(columns =>
                {
                    ...
                    if (hasTax) columns.ConstantColumn(40); // HSN/SAC
        ...
        Document.Create(container =>
        {
            ...
            container.Page(page =>
            {
                ...
                {
                    GeneratePOLayout(column, "VENDOR COPY");
                });
            ...
            container.Page(page =>
            {
                ...
                page.Content().Column(column =>
                {
                    GeneratePOLayout(column, "OFFICE COPY");
                });
